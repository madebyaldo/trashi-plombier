...
const TRUST_BADGES = ["Intervention 24h/24", "Arrivée en 30 min", "Devis gratuit", "10 ans d'expérience"];
...
export default function ZonePageTemplate({ zone }: { zone: ZonePageData }) {
  const relatedZoneLinks = zone.relatedZoneSlugs
    .map((slug) => {
      const related = getZoneBySlug(slug);
      if (related) return { href: `/${related.slug}`, label: `${related.type === "plombier" ? "Plombier" : "Chauffagiste"} ${related.city}` };
      return null;
    })
  ...
  const typeLabel = zone.type === "plombier" ? "Plombier" : "Chauffagiste";
  ...
  return (
    <>
        ...
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-[1fr_320px] gap-12 items-start">
            ...
            <div>
              ...
              >
                {typeLabel} à {zone.city} : Trashi Plombier
              </h2>
              <div className="space-y-5">
                {(zone.bodyParagraphs ?? []).map((para, i) => (
                  <p key={i} className="text-ink-600 text-[16px] leading-relaxed">
                    {para}
                  </p>
                ))}
                {(!zone.bodyParagraphs || zone.bodyParagraphs.length === 0) && (
                  <div className="bg-sand-50 border-2 border-dashed border-sand-300 rounded-lg p-8 text-center">
                    <p className="text-ink-500 font-medium">
                      Contenu local SEO à développer pour : {zone.city} ({zone.postalCode})
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* FAQ */}
      {zone.faqItems && zone.faqItems.length > 0 && (
        <section className="py-20 md:py-24 bg-white">
          <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
            ...
            <div className="max-w-[1000px] mx-auto">
              <ServiceFAQ items={zone.faqItems} />
            </div>
          </div>
        </section>
      )}
